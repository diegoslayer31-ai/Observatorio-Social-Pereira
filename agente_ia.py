...
def validar_documento_no_duplicado(numero_documento):
    """
    V16.69 - Protección contra duplicados por documento.
...
def _panel_medidas_activas_v1647(clave="medidas_activas"):
    ...
    # V16.69: no inflar el tablero por duplicados históricos exactos.
...
def gestion_usuarios():
 ...
 'ZEA': {'comuna': 'RIO OTUN', 'zona': 'URBANO'}}
        ...
        # V16.69 - La ficha individual usa EXACTAMENTE el mismo cálculo
...
# ============================================================
# V16.69 - REGLAS INSTITUCIONALES DE POSIBLE REINGRESO
# ============================================================
CRITERIOS_REINGRESO_V1641 = {
    # V16.69: la sanción empieza a contarse desde el DÍA SIGUIENTE
...
def _es_reingreso_operativo_v1667(documento, estado_anterior=""):
    """
    Regla institucional definitiva V16.69:
    ...
    - El estado actual, modalidad o ausencia de movimientos históricos
      NO convierten a una persona existente en ingreso nuevo.
    """
    ...
    try:
        ...
        total = int(df_existe.iloc[0]["total"] or 0)
        return total > 0
    except Exception:
        return False
...
def _compartir_foto_texto_movimiento_v1636(
    ...
    """
    Comparte la foto y el texto del reporte mediante el menú nativo del celular.
    En dispositivos compatibles, WhatsApp recibe la imagen con el texto como
    leyenda/caption. Si no hay soporte, deja respaldo con descarga temporal
    y botón de WhatsApp para el texto.
    """
    if not foto_bytes:
...
def gestion_usuarios_movil():
    ...
    # V16.69 - Los inspiradores también necesitan ver quién tiene
...
"""
    ...
    try:
        ...
        if not permiso_actual.empty:
            ...
            # "fuera con permiso". El permiso debe estar cerrado por la lógica V16.69.
            tiene_fuga_activa = False
            try:
                if not medida_activa.empty:
                    tiene_fuga_activa = (
                        str(medida_activa.iloc[0].get("motivo") or "")
                        .strip()
                        .upper()
                        == "FUGA"
                    )
    ...
    if accion == "➕ Ingreso / Reingreso":
        ...
        # V16.69 - Al estar en el flujo de usuario existente,
        # toda nueva llegada se registra como REINGRESO.
        tipo_previsto = "REINGRESO"
        st.info(
            "🔁 Esta cédula ya existe en la base maestra habitante_de_calle: "
            "se registrará como REINGRESO."
        )
        ...
        if st.button(
            ...
            if not confirmar:
                ...
                # V16.69 - Un usuario existente que salió y vuelve NO es
                # "ingreso nuevo". Se clasifica por su historial operativo.
                # V16.69 - Este flujo parte de "Buscar usuario existente".
    ...
    # --------------------------------------------------------
        ...
        st.caption(
            ...
            "No la marca como EGRESADA: pasa a INACTIVO y conserva todo su expediente e historial."
        )
            ...
            if not motivo_salida_vol.strip():
                ...
                try:
                    with engine.begin() as conn:
                        ...
                        # V16.69 - Una salida voluntaria significa que la persona
                ...
                registrar_auditoria(
                    ...
                    valor_nuevo="INACTIVO - FUERA DEL ALBERGUE - SALIDA VOLUNTARIA",
                ...
                st.success(
                    "✅ Salida voluntaria registrada. La persona quedó INACTIVA, liberó el cupo y conserva todo su expediente e historial."
                )
    ...
    # --------------------------------------------------------
        ...
        # V16.69 - Foto temporal opcional para sanción / fuga / expulsión.
            ...
            if not motivo.strip():
                ...
                if medida_existente:
                    ...
                    with engine.begin() as conn:
                        ...
                        # V16.69 - Si la causal es FUGA, el permiso abierto deja
                        ...
                        if causal_medida == "FUGA":
                            conn.execute(
                                text("""
                                    UPDATE permisos_usuarios
                                    SET estado_permiso = 'CERRADO',
                                        observacion_regreso = CONCAT(
                                            COALESCE(observacion_regreso, ''),
                                            CASE
                                                WHEN COALESCE(observacion_regreso,'') <> ''
                                                THEN ' | '
                                                ELSE ''
                                            END,
                                            'NO REGRESÓ - PERMISO CERRADO POR FUGA'
                                        ),
                                        cerrado_en = NOW()
                                    WHERE TRIM(
                                        CAST(numero_identificacion AS TEXT)
                    ...
                    detalle_reporte = motivo.strip()
                    if obs.strip():
                        detalle_reporte += (
                            (" | " if detalle_reporte else "")
                            + obs.strip()
                        )
                    ...
                    if causal_medida == "FUGA":
                        st.success(
                            "✅ Fuga registrada correctamente. "
                            "El permiso abierto fue cerrado automáticamente como NO REGRESÓ."
                        )
                    else:
...
def control_turno_v13():
    ...
    st.caption(
        ...
        f"Fecha operativa Colombia: {ahora_colombia().strftime('%d/%m/%Y')} · "
    ...
    # V16.69 - Base maestra completa para resolver permisos.
    try:
    ...
    # Movimientos del día
    # --------------------------------------------------------
    # V16.69 - "HOY" se define con la fecha local de Colombia.
    ...
    if not movimientos_hoy.empty:
        movimientos_hoy = movimientos_hoy.copy()
        movimientos_hoy["tipo_movimiento_norm"] = (
            movimientos_hoy["tipo_movimiento"]
            .fillna("")
            .astype(str)
            .str.upper()
            .str.strip()
        )
        movimientos_hoy["documento_norm"] = (
            movimientos_hoy["numero_identificacion"]
            .fillna("")
            .astype(str)
            .str.upper()
            .str.replace(r"[^A-Z0-9]", "", regex=True)
        ...
        # Una misma persona no se cuenta dos veces en el mismo tipo de evento
        # durante el mismo día, aunque por error operativo existan duplicados.
        mov_unicos_hoy = movimientos_hoy.drop_duplicates(
            subset=["documento_norm", "tipo_movimiento_norm"],
            keep="first"
        )

        ingresos_hoy = int(
            (mov_unicos_hoy["tipo_movimiento_norm"] == "INGRESO").sum()
        )
        reingresos_hoy = int(
            (mov_unicos_hoy["tipo_movimiento_norm"] == "REINGRESO").sum()
        )
        salidas_permiso_hoy = int(
            (mov_unicos_hoy["tipo_movimiento_norm"] == "SALIDA_PERMISO").sum()
        )
        regresos_permiso_hoy = int(
            (mov_unicos_hoy["tipo_movimiento_norm"] == "REGRESO_PERMISO").sum()
        )
    ...
    st.caption(
        "Activos = personas que actualmente ocupan cupo. "
        "Presentes = activos dentro del albergue. "
        "Con permiso = activos temporalmente fuera con permiso ABIERTO."
    )
    ...
    # Ausencias que NO son permisos abiertos (p. ej. salida voluntaria aún
    # reflejada operativamente). Se separan para no confundirlas con permisos.
    docs_permiso_abierto_norm = set()
    if not permisos_det.empty:
        docs_permiso_abierto_norm = set(
            permisos_det["documento"]
            .fillna("")
            .astype(str)
            .str.upper()
            .str.replace(r"[^A-Z0-9]", "", regex=True)
    ...
    fuera_otros = fuera.copy()
    if not fuera_otros.empty:
        fuera_otros["documento_norm"] = (
            fuera_otros["documento"]
            .fillna("")
            .astype(str)
            .str.upper()
            .str.replace(r"[^A-Z0-9]", "", regex=True)
        )
        fuera_otros = fuera_otros[
            ~fuera_otros["documento_norm"].isin(docs_permiso_abierto_norm)
        ]

    otras_ausencias_mod = (
        fuera_otros.groupby("modalidad")
        .size()
        .reset_index(name="otras_ausencias")
        if not fuera_otros.empty
        else pd.DataFrame(columns=["modalidad", "otras_ausencias"])
    )

    # V16.69 - "Con permiso" debe contar EXCLUSIVAMENTE permisos ABIERTOS.
    ...
    permisos_mod = (
        permisos_det[
            permisos_det["modalidad"].fillna("").astype(str).str.strip() != ""
        ]
        .groupby("modalidad")
        .size()
    ...
    ocupacion = (
        ...
        .merge(otras_ausencias_mod, on="modalidad", how="left")
    )
    ...
    for c in ["activos", "presentes", "con_permiso", "otras_ausencias"]:
...
def panel_profesional_v15(doc_forzado=None, incrustado=False):
    ...
    if rol_actual == "PROFESIONAL":
        # V16.69 - Todo funcionario con rol PROFESIONAL debe tener acceso completo al PAI.
        ...
        try:
            with engine.begin() as conn:
                # Mantener coherencia administrativa: un PROFESIONAL queda con acceso_pai habilitado.
                conn.execute(
                    text("""
                        UPDATE funcionarios_sistema
                        SET acceso_pai = TRUE
                ...
                if not vinculo_row:
                    # Buscar registro profesional por nombre normalizado.
                    prof_row = conn.execute(
                        text("""
                            SELECT id, nombre
                            FROM profesionales
                            WHERE UPPER(
                                REGEXP_REPLACE(TRIM(COALESCE(nombre,'')), '\\s+', ' ', 'g')
                            ) = UPPER(
                                REGEXP_REPLACE(TRIM(:nombre), '\\s+', ' ', 'g')
                            )
                            ORDER BY id
                            LIMIT 1
                        """),
                    ...
                    existe_map = conn.execute(
                        text("""
                            SELECT 1
                            FROM pai_profesional_funcionario
                            WHERE TRIM(CAST(cedula_funcionario AS TEXT)) = :cedula
                            LIMIT 1
                        """),
                        {"cedula": cedula_actual}
                    ).first()
                    ...
                    if existe_map:
                        conn.execute(
                            text("""
                                UPDATE pai_profesional_funcionario
                                SET profesional_id = :profesional_id,
                                    activo = TRUE
                                WHERE TRIM(CAST(cedula_funcionario AS TEXT)) = :cedula
                            """),
                            {
                                "profesional_id": prof_auto_id,
                                "cedula": cedula_actual
                            }
                        )
                    else:
                        conn.execute(
                            text("""
                                ...
                                VALUES (:cedula, :profesional_id, TRUE)
                            """),
                            {
                                "cedula": cedula_actual,
                                "profesional_id": prof_auto_id
            ...
            gestion = pd.read_sql(
                text("""
                    ...
                    LEFT JOIN LATERAL (
                        ...
                        ORDER BY
                            CASE
                                WHEN UPPER(TRIM(COALESCE(hx.estado_caso,'')))='ACTIVO' THEN 0
                                ELSE 1
                            END,
                            CASE
                                WHEN hx.modalidad IS NOT NULL
                                 AND TRIM(CAST(hx.modalidad AS TEXT))<>'' THEN 0
                                ELSE 1
                            END
                        LIMIT 1
                    ) h ON TRUE
    ...
    if incrustado and doc_forzado is not None:
        personas = pd.read_sql(
            text("""
                ...
                ORDER BY
                    TRIM(CAST(numero_identificacion AS TEXT)),
                    CASE
                        WHEN UPPER(TRIM(COALESCE(estado_caso,'')))='ACTIVO' THEN 0
                        ELSE 1
                    END,
                    CASE
        ...
        if personas.empty:
            st.warning(
                "La persona seleccionada no existe en la base maestra de habitantes de calle."
            )
            return
    else:
        try:
    ...
    if incrustado:
        ...
        if objetivos.empty:
            if objetivos_hist_2026.empty:
                ...
                st.info(
                    "Esta persona tiene objetivos migrados del PAI 2026. "
                    "Estos objetivos también son operativos: pueden recibir seguimiento, "
                    "modificación de avance y cierre por los profesionales autorizados."
                )
    ...
    with tab_seg:
        ...
        st.info(
            f"El seguimiento se guardará para **{nombre_usuario} · CC {doc_sel}**. "
            "Cualquier profesional autorizado puede hacer seguimiento a cualquier objetivo; "
            "el sistema conservará quién realizó cada intervención."
        )
                ...
                if not actividad.strip() or not descripcion_seg.strip():
                    ...
                    try:
                        registrar_auditoria(
                            ...
                            modulo="PAI Profesional",
                            valor_anterior=f"Objetivo #{objetivo_seg}",
                            valor_nuevo=f"Seguimiento por {prof_nombre}",
                            observacion=descripcion_seg.strip()[:500]
                        )
                    except Exception:
                        pass
    ...
    # ============================================================
    with tab_hist:
        st.markdown(f"### 📚 Trazabilidad PAI 2026 de {nombre_usuario}")
        st.caption(
            "Aquí se conserva la fuente migrada y los seguimientos originales. "
            "Los objetivos migrados también aparecen en Objetivos y pueden seguir gestionándose."
        )
...
def dashboard_ejecutivo():
    ...
    # ========================================================
    # V16.69 - CLASIFICACIÓN HISTÓRICA CORREGIDA DE INGRESOS / REINGRESOS
    # ========================================================
    # Regla:
    ...
    # V16.69 - Recuperar evidencia histórica de la base maestra.
    ...
    if not df_llegadas_hist.empty:
        # V16.69 - fecha_movimiento ya llega desde la consulta con la
        ...
        # V16.69 - Clasificación corregida para población histórica migrada.
...
with st.sidebar:
    ...
    if rol_menu == "INSPIRADOR":
        ...
        # V16.69 - Los profesionales tienen acceso directo al módulo PAI completo.
        if st.button(
        ...
        # V16.69 - El informe mensual también es parte del acceso profesional.
...
def formulario_genero_diversidad(doc_forzado=None, nombre_persona=None, incrustado=False):
    ...
    with st.form(f"form_genero_diversidad_{doc_forzado or 'manual'}"):
        ...
        # V16.69 - Se retira del formulario el campo "Expresión de género".
        ...
        expresion_genero = str(
            _gv("expresion_genero", "No informa") or "No informa"
        ).strip()
...
def modulo_auditoria_sesiones_v1634():
    ...
    # V16.69 - Los filtros se interpretan como días de Colombia.
    ...
    # V16.69 - fecha_hora se guarda con zona horaria en PostgreSQL.
